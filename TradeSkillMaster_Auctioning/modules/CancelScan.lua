...
function Cancel:ProcessItem(itemString, noLog)
	...
	for _, operation in pairs(operations) do
		...
		for i=GetNumAuctionItems("owner"), 1, -1 do
			if select(14, GetAuctionItemInfo("owner", i)) == 0 and itemString == TSMAPI:GetBaseItemString(GetAuctionItemLink("owner", i), true) then
...
function Cancel:SpecialScanProcessItem(itemString, noLog)
	...
	for i=GetNumAuctionItems("owner"), 1, -1 do
		if select(14, GetAuctionItemInfo("owner", i)) == 0 and itemString == TSMAPI:GetBaseItemString(GetAuctionItemLink("owner", i), true) then
			local _, _, quantity, _, _, _, _, bid, _, buyout, activeBid, _, _, wasSold = GetAuctionItemInfo("owner", i)
