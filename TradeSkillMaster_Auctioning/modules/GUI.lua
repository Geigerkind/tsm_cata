...
function private:CreateButtons(parent)
	...
	local button = TSMAPI.GUI:CreateButton(frame, 22, "TSMAuctioningCancelAllButton")
	button:SetPoint("TOPLEFT", TSMAuctioningCancelButton, "TOPLEFT", 0, 25)
	button:SetWidth(80)
	button:SetHeight(height)
...
function GUI:CreateSelectionFrame(parent)
	...
	local postBtn = TSMAPI.GUI:CreateButton(frame, 16, "TSMAuctioningStartPostScanButton")
	...
	local cancelBtn = TSMAPI.GUI:CreateButton(frame, 16, "TSMAuctioningStartCancelScanButton")
	...
	someObj.recPostFunc = function()
		TSMAPI:CreateTimeDelay("tryToPost", 0.2, function()
			if not private.buttons.stop.isDone then
				--TSMAuctioningPostButton:Click()
				if not private.buttons.stop:IsVisible() then
					return
				end
				
				someObj.recPostFunc()
			else 
				private.buttons.stop:Click()
				TSMAPI:CreateTimeDelay("tryToPost", 0.75, function() 
					if not RPLL_CONT_REPOST:IsVisible() then
						return
					end
					RPLL_CONT_REPOST:Click() 
				end)
			end
		end)
	end
	
	--local contRepost = TSMAPI.GUI:CreateButton(frame, 16, "RPLL_CONT_REPOST")
	--contRepost:SetPoint("BOTTOMLEFT", cancelBtn, "BOTTOMRIGHT", 5, -25)
	--contRepost:SetHeight(20)
	--contRepost:SetWidth(btnWidth)
	--contRepost:SetText("Repost continuously")
	--contRepost:SetScript("OnClick", function()
	--		TSMAuctioningStartPostScanButton:Click()
	--		private.mode = "Post"
	--		private.specialMode = nil
	--		someObj.recPostFunc()
	--	end)
	--frame.contRepost = contRepost
